...
- [ ] **B7 End-to-end test** — _Unblocked: PR #49 merged 2026-06-04T07:57Z as `41f8fb4` on main (rebase-merge)._ Bump `version.json` to `1.3.4-rc1` (using `release.yml`), tag `v1.3.4-rc1`. Confirm `publish-apt-repo` runs to completion, gh-pages picks up the `.deb` + `Release`/`InRelease`. Install on a fresh Ubuntu VM via the apt source from Part 3 of the guide. Optional follow-up `v1.3.4-rc2` to verify `apt upgrade`. (Strategy chosen 2026-06-04 over throwaway `v0.0.0-apt-test` to avoid filename collision — `build-deb.sh` derives the deb name from `version.json`, not the tag.)
