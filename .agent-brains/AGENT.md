...
- **Theming / DynamicResource**: The app theme is **Material.Avalonia** (`App.axaml` → `MaterialTheme BaseTheme="Dark"`) — NOT Semi, despite the `SemiColor*` key names. `SemiColorDanger/Warning/Border` are **hand-defined app resources** in `App.axaml` `Application.Resources`; any new `{DynamicResource X}` reference MUST have its key defined there (or by the Material theme). `DynamicResource` fails **silently** on missing keys — no error, the setter just doesn't apply — so dangling keys ship invisible styling bugs that tests can't catch (found in PR #61 review). Code-behind lookups should use a fallback: `TryFindResource(key, ...) ?? fixedBrush`.
